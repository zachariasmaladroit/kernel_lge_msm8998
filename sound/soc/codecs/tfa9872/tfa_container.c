 ...
 */
struct tfa_livedata_list *tfa_cont_get_dev_livedata_list(struct tfa_container * cont, int dev_idx,
					   int lifedata_idx)
{
	struct tfa_device_list *dev;
	int idx, hit;
 ...
 */
static enum tfa98xx_error tfa_cont_write_vstep(int dev_idx,  struct tfa_volume_step2_file *vp, int vstep)
{
...
enum tfa98xx_error tfa_cont_write_regs_dev(int dev_idx) {
	...
	for(i=0;i<dev->length;i++) {
		...
		if  ( dev->list[i].type == dsc_register ) {
			err = tfa_run_write_register( dev_idx, (struct tfa_reg_patch *)
...
enum tfa98xx_error  tfa_cont_write_regs_prof(int dev_idx, int prof_idx) {
	...
	for(i=0;i<prof->length;i++) {
		...
		if  ( prof->list[i].type == dsc_register ) {
			err = tfa_run_write_register( dev_idx, (struct tfa_reg_patch *)( prof->list[i].offset+(char*)g_cont));
		}
		if ( err ) break;
	}
	return err;
...
enum tfa98xx_error get_sample_rate_info(int dev_idx, struct tfa_profile_list *prof, struct tfa_profile_list *previous_prof, int fs_previous_profile)
{
	...
	for(i=0;i<previous_prof->length;i++) {
		...
		if(i == 0) {
			while(previous_prof->list[i].type != dsc_default && i < previous_prof->length) {
				i++;
			}
			i++;
		}
...
struct tfa_livedata_list* tfa_cont_next_livedata(struct tfa_livedata_list* livedata) {
	struct tfa_livedata_list* nextlivedata = (struct tfa_livedata_list *)( (char*)livedata + (livedata->length*4) + 
...
int tfa_tib_dsp_msgblob(int devidx, int length, const char *buffer)
{
	...
	*blobptr++ = (uint8_t)length; //lsb
	...
	if((buf[0] == SB_PARAM_SET_RE25C && buf[1] == 0x81 && buf[2] == 0x00 )
		...
		|| (buf[0] == SB_PARAM_GET_ALGO_PARAMS && buf[1] == 0x81 && buf[2] == 0x04)
		|| (buf[0] == BFB_PAR_ID_GET_COEFS && buf[1] == 0x82)
		|| (buf[0] == SB_PARAM_GET_RE25C && buf[1] == 0x81 &&  buf[2] == 0x04))
	{

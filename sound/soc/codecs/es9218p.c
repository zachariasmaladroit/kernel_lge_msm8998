...
static int es9218p_sabre_amp_start(struct i2c_client *client, int headset)
{
    ...
    switch(headset) {
        ...
        default :
            pr_err_ratelimited("%s() : Unknown headset = %d \n", __func__, headset);
            ret = 1;
            break;
    }
                   ...
                   const char *buf, size_t count)
{
    ...
    if (input_filter > 11) {
        pr_err_ratelimited("%s() : Invalid filter = %d return \n", __func__, input_filter);
        return 0;
    }
...
static int es9218_sabre_cfg_custom_filter(struct sabre_custom_filter *sabre_filter)
{
    ...
    if (rc < 0) {
        pr_err_ratelimited("%s: rc = %d return ",__func__, rc);
        return rc;
    }
        ...
        struct snd_ctl_elem_value *ucontrol)
{
	...
	if (g_headset_type == 1 && force_advanced_mode) {
		pr_info_ratelimited("%s(): jollaman999 mod: Force change to Advanced Mode!\n", __func__);
		g_headset_type = 2;
	}
        ...
        */
        pr_err_ratelimited("%s() : invalid headset type = %d, state = %s\n", __func__, value, power_state[es9218_power_state]);
        return 0;
	}
...
static int lge_ess_fade_inout_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    ...
    if(lge_ess_fade_inout_init != true) {
        ...
        if(!mute_work) {
            ...
            pr_err_ratelimited("%s() : devm_kzalloc failed!!\n", __func__);
            return 0;
        }
        ...
        struct es9218_data *pdata)
{
    ...
    if (IS_ERR(pdata->vreg_dvdd)) {
		...
		if (PTR_ERR(pdata->vreg_dvdd) == -EPROBE_DEFER) {
			pr_err_ratelimited("In %s, vreg_dvdd probe defer\n", __func__);
    ...
    if (ret < 0) {
        pr_err_ratelimited("%s Invalid property name\n",__func__);
...
static int es9218_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
    ...
    if (!i2c_check_functionality(client->adapter,
        ...
        pr_err_ratelimited("%s: no support for i2c read/write byte data\n", __func__);
        return -EIO;
    }
    ...
    if (ret < 0) {
        pr_err_ratelimited("%s(): debugger gpio request failed\n", __func__);

...
static void msm_qti_pp_send_eq_values_(int eq_idx)
{
	...
	if (ac == NULL) {
		pr_err_ratelimited("%s: Could not get audio client for session: %d\n",
	...
	if (result < 0)
		pr_err_ratelimited("%s: Call to ASM equalizer failed, returned = %d\n",
		      __func__, result);
done:
	return;
}
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	if (fe_id >= MSM_FRONTEND_DAI_MM_SIZE) {
		pr_err_ratelimited("%s: invalid FE %d\n", __func__, fe_id);
		return -EINVAL;
	}
	...
	if (ac == NULL) {
		pr_err_ratelimited("%s ac is null.\n", __func__);
		ret = -EINVAL;
		goto done;
	}
						...
						uint16_t op_FR_ip_FR_weight)
{
	...
	*update_params_value16++ = op_FR_ip_FR_weight;
	...
	if (params_length) {
		...
		if (rc) {
			pr_err_ratelimited("%s: send params failed rc=%d\n", __func__, rc);
			kfree(params_value);
			return -EINVAL;
		}
	}
	...
	return 0;
skip_send_cmd:
		pr_err_ratelimited("%s: insufficient memory, send cmd failed\n",
			__func__);
		kfree(params_value);
		return -ENOMEM;
}
					    ...
					    struct snd_ctl_elem_value *ucontrol)
{
	...
	if ((be_idx >= MSM_BACKEND_DAI_MAX) || !msm_bedai.active) {
		pr_err_ratelimited("%s, back not active to query rms be_idx:%d\n",
			__func__, be_idx);
		rc = -EINVAL;
	...
	if ((copp_idx < 0) || (copp_idx > MAX_COPPS_PER_PORT)) {
		pr_err_ratelimited("%s, no active copp to query rms copp_idx:%d\n",
			__func__ , copp_idx);
		rc = -EINVAL;
		goto get_rms_value_err;
	}
	...
	if (rc) {
		pr_err_ratelimited("%s: get parameters failed rc=%d\n", __func__, rc);
		rc = -EINVAL;
		goto get_rms_value_err;
	}
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ret)
		pr_err_ratelimited("%s: failed to set LB vol for SLIMBUS_7_TX, err %d\n",
			__func__, ret);
	else
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ret)
		pr_err_ratelimited("%s: failed to set LB vol for SLIMBUS_8_TX", __func__);
	else
...
static int msm_qti_pp_asphere_send_params(int port_id, int copp_idx, bool force)
{
	...
	if (set_strength) {
		...
		ret = q6common_pack_pp_params(packed_params +
					      ...
					      &param_size);
		if (ret) {
			pr_err_ratelimited("%s: Failed to pack params, error %d\n",
			       __func__, ret);
			goto done;
		}
	...
	ret = adm_set_pp_params(port_id, copp_idx, NULL, packed_params,
				packed_params_size);
	if (ret)
		pr_err_ratelimited("%s: set param failed with err=%d\n", __func__, ret);
...
int msm_qti_pp_asphere_init(int port_id, int copp_idx)
{
	...
	if (index < 0) {
		pr_err_ratelimited("%s: Invalid port idx %d port_id %#x\n", __func__, index,
			port_id);
		return -EINVAL;
	}
...
int msm_adsp_init_mixer_ctl_pp_event_queue(struct snd_soc_pcm_runtime *rtd)
{
	...
	kfree(mixer_str);
	if (!kctl) {
		pr_err_ratelimited("%s: failed to get kctl.\n", __func__);
		ret = -EINVAL;
		goto done;
	}

	if (kctl->private_data != NULL) {
		pr_err_ratelimited("%s: kctl_prtd is not NULL at initialization.\n",
			__func__);
		return -EINVAL;
	}
			...
			uint32_t *payload)
{
	...
	if (!rtd || !payload) {
		pr_err_ratelimited("%s: %s is NULL\n", __func__,
			(!rtd) ? "rtd" : "payload");
		ret = -EINVAL;
		goto done;
	}
	...
	if (rtd->card->snd_card == NULL) {
		pr_err_ratelimited("%s: snd_card is null.\n", __func__);
		ret = -EINVAL;
		goto done;
	}
	...
	if (event_data->payload_len < sizeof(struct msm_adsp_event_data)) {
		pr_err_ratelimited("%s: event_data size of %x is less than expected.\n",
	...
	if (event_data->payload_len >
		...
		pr_err_ratelimited("%s: payload length exceeds limit of %u bytes.\n",
	...
	if (kctl_prtd == NULL) {
		...
		ret = -EINVAL;
		pr_err_ratelimited("%s: event queue is not initialized.\n", __func__);
		goto done;
	}
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	if ((copp_idx < 0) || (copp_idx > MAX_COPPS_PER_PORT)) {
		pr_err_ratelimited("%s : no active copp to query multichannel ec copp_idx: %u\n",
			__func__, copp_idx);
		return -EINVAL;
	}

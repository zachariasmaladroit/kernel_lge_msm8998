...
static void lge_time_to_full_evaluate_report(long eoc, struct lge_time_to_full_data *ttf) {
	...
	rc = power_supply_get_property(ttf->usb_psy, POWER_SUPPLY_PROP_REAL_TYPE, &prop);
	if (rc)
...
static void lge_battery_profile_read_work(struct work_struct *work)
{
	...
	char cc_data_name[22] = "lge,cc_data-signature";
	...
	for_each_child_of_node(node, child) {
		...
		if (!strcmp(ttf->profile_name, profile_name) && match_chargerlogo) {
			...
			kzfree(ttf->cc_data);
			ttf->cc_data = NULL;
			if (lge_get_hydra_mode() != LGE_HYDRA_MODE_SIGNATURE)
				strcpy(cc_data_name, "lge,cc_data");
			if (of_get_property(child, cc_data_name, &len)) {
				ttf->cc_data = kzalloc(len, GFP_KERNEL);
				ttf->cc_data_length = len / sizeof(struct cc_step);
				rc = of_property_read_u32_array(child, cc_data_name,

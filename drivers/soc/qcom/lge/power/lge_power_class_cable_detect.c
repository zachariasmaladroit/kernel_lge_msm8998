...
static void lge_cable_detect_external_power_changed(struct lge_power *lpc)
{
	...
	if(!cd->usb_psy){
		...
		if (cd->is_factory_cable) {
			if (cd->chg_type == POWER_SUPPLY_TYPE_USB_HVDCP ||
			...
			rc = cd->usb_psy->desc->get_property(
				cd->usb_psy, POWER_SUPPLY_PROP_SDP_CURRENT_MAX,
				&ret);
		...
		} else if (cd->chg_type == POWER_SUPPLY_TYPE_USB_FLOAT) {
			cd->modified_usb_ma = cd->usb_current*1000;
			cd->modified_ibat_ma = cd->ibat_current*1000;
...
static void get_cable_data_from_dt(struct cable_detect *cd)
{
	...
	const char *propname_renewal[MAX_CABLE_NUM] = {
		"lge,no-init-cable-renewal",
		"lge,cable-mhl-1k-renewal",
		"lge,cable-u-28p7k-renewal",
		"lge,cable-28p7k-renewal",
		"lge,cable-56k-renewal",
		"lge,cable-100k-renewal",
		"lge,cable-130k-renewal",
		"lge,cable-180k-renewal",
		"lge,cable-200k-renewal",
		"lge,cable-220k-renewal",
		"lge,cable-270k-renewal",
		"lge,cable-330k-renewal",
		"lge,cable-620k-renewal",
		"lge,cable-910k-renewal",
		"lge,cable-none-renewal"
	};
	...
	for (i = 0 ; i < MAX_CABLE_NUM ; i++) {
		...
		if (lge_get_hydra_mode() == LGE_HYDRA_MODE_RENEWAL) {
			of_property_read_u32_array(node_temp, propname_renewal[i],
								cable_value, 5);
		} else {

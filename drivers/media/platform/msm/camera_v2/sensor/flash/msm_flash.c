...
{
	if (!torch_trigger) {
		pr_err_ratelimited("No torch trigger found, can't set brightness\n");
		return;
	}
				...
				void *data)
{
	...
	if (!fctrl) {
		pr_err_ratelimited("Invalid fctrl\n");
		return -EINVAL;
	}
	...
	for (i = 0; i < fctrl->torch_num_sources; i++) {
		if (fctrl->torch_trigger[i]) {
			...
			rc = led_classdev_register(&pdev->dev,
				&msm_torch_led[i]);
			if (rc) {
				pr_err_ratelimited("Failed to register %d led dev. rc = %d\n",
						i, rc);
				return rc;
			}
		} else {
			pr_err_ratelimited("Invalid fctrl->torch_trigger[%d]\n", i);
			return -EINVAL;
		}
	}
	...
	struct msm_camera_i2c_reg_setting_array *settings)
{
	...
	if ((!conf_array.size) || (conf_array.size > MAX_I2C_REG_SET)) {
		pr_err_ratelimited("failed: invalid size %d", conf_array.size);
		return -EINVAL;
	}
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	if (!flash_init_info || !flash_init_info->power_setting_array) {
		pr_err_ratelimited("%s:%d failed: Null pointer\n", __func__, __LINE__);
		return -EFAULT;
	}
	...
	if (is_compat_task()) {
		...
		if (!power_setting_array32) {
			pr_err_ratelimited("%s mem allocation failed %d\n",
				__func__, __LINE__);
			return -ENOMEM;
		}
		...
		if ((!flash_ctrl->power_setting_array.size) ||
			...
			pr_err_ratelimited("failed: invalid size %d, size_down %d",
				flash_ctrl->power_setting_array.size,
				flash_ctrl->power_setting_array.size_down);
			kfree(power_setting_array32);
	...
	if (rc < 0) {
		pr_err_ratelimited("%s msm_camera_power_up failed %d\n",
			__func__, __LINE__);
		goto msm_flash_i2c_init_fail;
	}
	...
	if (flash_data->cfg.flash_init_info->settings) {
		...
		if (rc < 0) {
			pr_err_ratelimited("%s:%d msm_flash_i2c_write_table rc %d failed\n",
	...
	struct msm_flash_ctrl_t *flash_ctrl)
{
	if (msm_camera_power_down(&flash_ctrl->power_info,
		...
		pr_err_ratelimited("%s msm_camera_power_down failed %d\n",
			__func__, __LINE__);
		return -EINVAL;
	}
	return 0;
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	if (flash_ctrl->flash_state == MSM_CAMERA_FLASH_INIT) {
		pr_err_ratelimited("%s:%d Invalid flash state = %d",
	...
	if (flash_driver_type == FLASH_DRIVER_DEFAULT) {
		pr_err_ratelimited("%s:%d invalid flash_driver_type", __func__, __LINE__);
		return -EINVAL;
	}
	...
	if (rc < 0) {
		pr_err_ratelimited("%s:%d failed invalid flash_driver_type %d\n",
			__func__, __LINE__,
	...
	if (flash_ctrl->func_tbl->camera_flash_init) {
		...
		if (rc < 0) {
			pr_err_ratelimited("%s:%d camera_flash_init failed rc = %d",
				__func__, __LINE__, rc);
			return rc;
		}
	}
	...
	struct msm_flash_ctrl_t *flash_ctrl)
{
	...
	if (flash_ctrl->switch_trigger == NULL) {
		pr_err_ratelimited("%s:%d Invalid argument\n",
				__func__, __LINE__);
		return -EINVAL;
	}
	...
	if (flash_ctrl->flash_state == MSM_CAMERA_FLASH_INIT &&
		...
		if (ret < 0) {
			pr_err_ratelimited("%s:%d regulator enable failed ret = %d\n",
		...
		if (ret < 0) {
			pr_err_ratelimited("%s:%d regulator disable failed ret = %d\n",
				__func__, __LINE__, ret);
			return ret;
		}
		flash_ctrl->is_regulator_enabled = 0;
	...
	struct msm_flash_query_data_t *flash_query_data)
{
	...
	if (flash_ctrl->switch_trigger) {
		...
		if (ret < 0) {
			pr_err_ratelimited("%s:%d Query max_avail_curr failed ret = %d\n",
				__func__, __LINE__, ret);
			return ret;
		}
	}
	...
	void __user *argp)
{
	...
	if (rc < 0) {
		pr_err_ratelimited("%s:%d Enable/Disable Regulator failed ret = %d",
			__func__, __LINE__, rc);
		return rc;
	}
	...
	struct msm_flash_ctrl_t *fctrl)
{
	...
	if (!switch_src_node) {
		...
		if (rc < 0) {
			rc = of_property_read_string(switch_src_node,
				...
				&fctrl->switch_trigger_name);
			if (rc < 0)
				pr_err_ratelimited("default-trigger read failed\n");
		}
	...
	if (of_get_property(of_node, "qcom,flash-source", &count)) {
		...
		if (count > MAX_LED_TRIGGERS) {
			pr_err_ratelimited("invalid count\n");
			return -EINVAL;
		}
		...
		for (i = 0; i < count; i++) {
			flash_src_node = of_parse_phandle(of_node,
				"qcom,flash-source", i);
			if (!flash_src_node) {
				pr_err_ratelimited("flash_src_node NULL\n");
				continue;
			}
			...
			if (rc < 0) {
				rc = of_property_read_u32(flash_src_node,
					...
					&fctrl->flash_op_current[i]);
				if (rc < 0) {
					pr_err_ratelimited("current: read failed\n");
					of_node_put(flash_src_node);
					continue;
				}
			}
			...
			if (rc < 0) {
				rc = of_property_read_u32(flash_src_node,
					...
					&fctrl->flash_max_duration[i]);
				if (rc < 0) {
					pr_err_ratelimited("duration: read failed\n");
					of_node_put(flash_src_node);
				}
	...
	if (of_get_property(of_node, "qcom,torch-source", &count)) {
		...
		for (i = 0; i < count; i++) {
			torch_src_node = of_parse_phandle(of_node,
				"qcom,torch-source", i);
			if (!torch_src_node) {
				pr_err_ratelimited("torch_src_node NULL\n");
				continue;
			}
	...
	struct msm_flash_ctrl_t *fctrl)
{
	...
	if (rc < 0) {
		pr_err_ratelimited("%s:%d msm_flash_get_pmic_source_info failed rc %d\n",
			__func__, __LINE__, rc);
		return rc;
	}
	...
	if (rc < 0) {
		pr_err_ratelimited("%s:%d msm_sensor_driver_get_gpio_data failed rc %d\n",
			__func__, __LINE__, rc);
		return rc;
	}
	...
	struct file *file, unsigned int cmd, void *arg)
{
	...
	if (!file || !arg) {
		pr_err_ratelimited("%s:failed NULL parameter\n", __func__);
		return -EINVAL;
	}
	...
	case VIDIOC_MSM_FLASH_CFG32:
		...
		break;
	case VIDIOC_MSM_FLASH_CFG:
		pr_err_ratelimited("invalid cmd 0x%x received\n", cmd);
		return -EINVAL;
	default:
...
static int32_t msm_flash_platform_probe(struct platform_device *pdev)
{
	...
	if (rc < 0) {
		pr_err_ratelimited("%s:%d msm_flash_get_dt_data failed\n",
			__func__, __LINE__);
		kfree(flash_ctrl);
		return -EINVAL;
	}
...
static int __init msm_flash_init_module(void)
{
	...
	if (rc)
		pr_err_ratelimited("platform probe for flash failed");

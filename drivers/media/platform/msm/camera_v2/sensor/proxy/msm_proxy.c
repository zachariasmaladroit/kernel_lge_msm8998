...
// for calibration
...
#ifdef COMPATIBILITY_CUT_1_0_CUT_1_1
	pr_debug("OffsetCalibration start!\n");
    ...
    if (moduleVersion == 1) // cut 1.1
	{
		...
		if ((offsetComp < 31000) && (offsetComp > (-63000)) && (Status == VL53L0_ERROR_NONE) ) {
			pr_debug("OffsetCalibration: spec in!\n");
		} else {
			VL53L0_RestoreOffset(Dev, st_offset);
			pr_debug("OffsetCalibration: spec out!\n");
		}
		pr_debug("offsetComp:%d OffsetCalibrationDataMicroMeter: %d!\n",offsetComp, st_offset);
    }
	...
	pr_debug("OffsetCalibration end!\n");
	return offset;
#else
	//VL53L0_GetOffsetCalibrationDataMicroMeter(Dev, &OffsetCalibrationDataMicroMeter);
	Status = VL53L0_PerformOffsetCalibration(Dev, 200<<16, &offsetComp); //(100) is 100mm target dis, &offsetComp is return
...
/* LGE_CHANGE_E, for initialization time reduce(spad calibration), 2016-02-13, seonyung.kim@lge.com */
				...
				pr_debug("write offset = %x to eeprom\n", offset);
				...
				if ((offset < 11) && (offset > (-21))) {
					...
					//if ((moduleId == 0x00) || (moduleId == 0x01) || (moduleId == 0x02)) {
						...
						pr_debug("VL53L0 VhvSettings:%d, PhaseCal:%d, isApertureSpads:%d\n",VhvSettings, PhaseCal, isApertureSpads);
						...
						pr_debug("KSY read inot cal count = %x to eeprom\n", finVal);
						pr_debug("KSY read inot offset = %x to eeprom\n", offset);
					//}
		...
		if (proxy_struct->i2c_fail_cnt >= proxy_struct->max_i2c_fail_thres) {
			pr_debug("proxy workqueue force end due to i2c fail!\n");
			break;
		}
		msleep(53);
...
{
	pr_debug("msm_proxy_thread_start\n");
	...
	if (msm_proxy_t.exit_workqueue) {
		...
		if (!msm_proxy_t.work_thread) {
			pr_debug("creating work_thread fail!\n");
			return 1;
		}
...
{
	uint16_t ret = 0;
	pr_debug("msm_proxy_thread_pause\n");
	ret = pause_proxy();
	return ret;
}
...
{
	uint16_t ret = 0;
	pr_debug("msm_proxy_thread_restart\n");
...
int32_t msm_init_proxy(void)
{
	...
	if ((shiftModuleId == 0x00) || (shiftModuleId == 0x01) || (shiftModuleId == 0x02)) {
		...
		pr_debug("inot read offset = %d from eeprom\n", offsetByte);
	...
	void __user *argp)
{
	...
	switch (cdata->cfgtype) {
      case CFG_PROXY_INIT:
		...
		if (rc < 0)
			    pr_debug("msm_proxy_init failed %d\n", rc);
        break;
      case CFG_PROXY_ON:{
        rc = msm_init_proxy();
        pr_debug("%s: Proxy is on! error_code = %d  \n", __func__, rc);
        break;
      }
      case CFG_GET_PROXY:{
      ...
      case CFG_PROXY_POWERUP:
        ...
        if (rc < 0)
          pr_debug("Failed proxy power up%d\n", rc);
        break;
      case CFG_PROXY_POWERDOWN:
		rc = msm_proxy_power_down(proxy_ctrl);
	if (rc < 0)
		pr_debug("msm_proxy_power_down failed %d\n", rc);
        break;
	default:
		break;
	}
	...
	const struct i2c_device_id *id)
{
	...
	if (client == NULL) {
		pr_debug("msm_ois_i2c_probe: client is null\n");
		return -EINVAL;
	}
	...
	proxy_ctrl_t = kzalloc(sizeof(struct msm_proxy_ctrl_t),
		GFP_KERNEL);
	if (!proxy_ctrl_t) {
		pr_debug("%s:%d failed no memory\n", __func__, __LINE__);
		return -ENOMEM;
	}

	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		pr_debug("i2c_check_functionality failed\n");
		goto probe_failure;
	}
...
//	uint8_t isApertureSpads = 0;
	...
	pr_debug("msm_init_proxy_EwokAPI(): Starting...\n");
...
/* LGE_CHANGE_S, for initialization time reduce(spad calibration), 2016-02-15, seonyung.kim@lge.com */
					...
					if(VhvSettings == 255 /* EEPROM default value */
						...
						pr_debug("VL53L0 count:%d, count_sep1:%d, count_sep2:%d(not calibrated!!)\n",count, count_sep1, count_sep2);
						pr_debug("VL53L0 VhvSettings:%d, PhaseCal:%d, isApertureSpads:%d(not calibrated!!)\n",VhvSettings_8, PhaseCal_8, isApertureSpads_8);
					}
					else {
...
//		if ((moduleId == 0x00) || (moduleId == 0x01) || (moduleId == 0x02)) {
		...
		msm_proxy_t.proxy_stat.cal_count = calCount;
		pr_debug("read offset = %d from eeprom\n", e2p_offset);
		...
		pr_debug("e2p_offset = %d, st_offset = %d, sensor_offset = %d\n", e2p_offset, st_offset, sensor_offset);
	}
	else if(moduleVersion == 0)
	{
	//readRangeOffset
...
/* LGE_CHANGE_E, changed LD clk to reduce the S5K2P7 sensor power noise. 2016-11-21 sungmin.cho@lge.com */
	...
	if (Status != VL53L0_ERROR_NONE)
	{
		pr_debug("msm_init_proxy_EwokAPI(): VL53L0_GetMeasurementDataReady() failed! %d %d\n", Status, msm_proxy_t.check_init_finish);
		return -1;
	} else
		msm_proxy_t.check_init_finish = 2; // Init Complete

	pr_debug("msm_init_proxy_EwokAPI(): End!\n");
	return 0;
}
#endif
...
int msm_init_proxy_BBearAPI(void)
{
	...
	pr_debug("msm_init_proxy_BBearAPI() ENTER!\n");
...
/*	if(ret != 0)
	{
		pr_debug("msm_init_proxy(), VL53LO_GetRangeStatus() fail!\n");
		return -1;
	}*/

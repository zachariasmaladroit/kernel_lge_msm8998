	...
	struct msm_vfe_stats_stream *stream_info)
{
	...
	if (stream_info->num_isp == 0) {
		...
		if (stream_info->vfe_mask & (1 << vfe_dev->pdev->id)) {
			pr_err_ratelimited("%s: stats %d already requested for vfe %d\n",
		...
		if (stream_info->framedrop_period != framedrop_period)
			rc = -EINVAL;
		if (rc) {
			pr_err_ratelimited("%s: Stats stream param mismatch between vfe\n",
				__func__);
			return rc;
		}
	}
...
int msm_isp_release_stats_stream(struct vfe_device *vfe_dev, void *arg)
{
	...
	if (vfe_idx == -ENOTTY || stream_info->stream_handle[vfe_idx] !=
		...
		pr_err_ratelimited("%s: Invalid stream handle %x, expected %x\n",
		...
		int active)
{
	...
	if (rc <= 0) {
		rc = rc ? rc : -ETIMEDOUT;
		pr_err_ratelimited("%s: wait for stats stream %x idx %d state %d active %d config failed %d\n",
	...
	struct msm_vfe_stats_stream_cfg_cmd *stream_cfg_cmd)
{
	...
	for (i = 0; i < stream_cfg_cmd->num_streams; i++) {
		...
		if (vfe_idx == -ENOTTY || stream_info->stream_handle[vfe_idx] !=
			...
			pr_err_ratelimited("%s: Invalid stream handle: 0x%x received expected %x\n",

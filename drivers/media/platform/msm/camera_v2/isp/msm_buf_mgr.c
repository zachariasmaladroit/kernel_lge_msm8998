	...
	uint32_t stream_id)
{
	...
	if (!bufq) {
		pr_err_ratelimited("%s: Invalid bufq, stream id %x\n",
			__func__, stream_id);
		return -EINVAL;
	}

	if (qbuf_buf->num_planes > MAX_PLANES_PER_STREAM) {
		pr_err_ratelimited("%s: Invalid num_planes %d , stream id %x\n",
	...
	struct msm_isp_buf_request_ver2 *buf_request)
{
	...
	if (!buf_request->handle) {
		pr_err_ratelimited("Invalid buffer handle\n");
		return -EINVAL;
	}

	bufq = msm_isp_get_bufq(buf_mgr, buf_request->handle);
	if (!bufq) {
		pr_err_ratelimited("%s: Invalid bufq stream_id %x\n",
	...
	if (!bufq->bufs) {
		pr_err_ratelimited("No free memory for buf info\n");
	...
	unsigned long fault_addr)
{
	...
	pr_err_ratelimited("%s: ==== SMMU page fault addr %lx ====\n", __func__,
		fault_addr);
	pr_err_ratelimited("%s: nearby stream id %x, frame_id %d\n", __func__,

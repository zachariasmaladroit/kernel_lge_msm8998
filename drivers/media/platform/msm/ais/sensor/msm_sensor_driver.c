...
static int msm_sensor_suspend(struct device *dev)
{
	...
	if (!s_ctrl) {
		pr_err_ratelimited("%s:%d Invalid platform data\n", __func__,
			__LINE__);
		return -EFAULT;
	}
...
static void bridge_irq_delay_work(struct work_struct *work)
{
	...
	if (!sensor_i2c_client || !slave_info || !sensor_name) {
		pr_err_ratelimited("%s:%d failed: %pK %pK %pK\n",
	...
	struct msm_sensor_info_t *probed_info, char *entity_name)
{
	...
	if (s_ctrl->is_probe_succeed == 1) {
		...
		if (slave_info->sensor_id_info.sensor_id ==
			...
			pr_err_ratelimited("slot%d: sensor id%d already probed\n",
				slave_info->camera_id,
				s_ctrl->sensordata->cam_slave_info->
					sensor_id_info.sensor_id);
...
CSID_TG:
	...
	*/
	...
	if (slave_info->gpio_intr_config.gpio_num != -1) {
		...
		if (rc < 0) {
			pr_err_ratelimited("%s: Failed to request irq_gpio %d",
				__func__, rc);
			goto cancel_work;
		}
		...
		if (gpio_is_valid(s_ctrl->gpio_array[0].gpio)) {
			rc |= gpio_direction_input(
				s_ctrl->gpio_array[0].gpio);
			if (rc) {
				pr_err_ratelimited("%s: Failed gpio_direction irq %d",
						__func__, rc);
				goto cancel_work;
			}
		}
		...
		if (s_ctrl->irq) {
			rc = request_irq(s_ctrl->irq, bridge_irq,
					...
					"qcom,camera", s_ctrl);
			if (rc) {
				pr_err_ratelimited("%s: Failed request_irq %d",
						__func__, rc);
				goto cancel_work;
			}

		} else {
			pr_err_ratelimited("%s: Failed gpio_to_irq %d",
				__func__, rc);
			rc = -EINVAL;
			goto cancel_work;
		}
...
static int32_t msm_sensor_driver_platform_probe(struct platform_device *pdev)
{
	...
	if (rc < 0) {
		pr_err_ratelimited("failed: camera create v4l2 rc %d", rc);
		goto FREE_S_CTRL;
	}

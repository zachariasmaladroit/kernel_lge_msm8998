...
#define EARLY_CAM_DBG(fmt, args...) pr_err_ratelimited(fmt, ##args)
#else
...
int msm_early_cam_disable_clocks(void)
{
	...
	if (new_early_cam_dev == NULL) {
		rc = -EINVAL;
		pr_err_ratelimited("%s: clock structure uninitialised %d\n", __func__,
			rc);
		return rc;
	}
	...
	if ((new_early_cam_dev->pdev == NULL) ||
		...
		rc = -EINVAL;
		pr_err_ratelimited("%s: Clock details uninitialised %d\n", __func__,
			rc);
		return rc;
	}
	...
	if (rc < 0) {
		pr_err_ratelimited("%s: clk disable failed %d\n", __func__, rc);
		return rc;
	}
	...
	if (rc < 0) {
		pr_err_ratelimited("%s: failed to vote OFF AHB_CLIENT_CSIPHY %d\n",
			__func__, rc);
		return rc;
	}
	...
	rc = cam_config_ahb_clk(NULL, 0, CAM_AHB_CLIENT_CSID,
		CAM_AHB_SUSPEND_VOTE);
	if (rc < 0) {
		pr_err_ratelimited("%s: failed to vote OFF AHB_CLIENT_CSID %d\n",
			__func__, rc);
		return rc;
	}
	...
	rc = cam_config_ahb_clk(NULL, 0, CAM_AHB_CLIENT_CCI,
		CAM_AHB_SUSPEND_VOTE);
	if (rc < 0) {
		pr_err_ratelimited("%s: failed to vote OFF AHB_CLIENT_CCI %d\n",
			__func__, rc);
		return rc;
	}
	...
	rc = cam_config_ahb_clk(NULL, 0, CAM_AHB_CLIENT_ISPIF,
		CAM_AHB_SUSPEND_VOTE);
	if (rc < 0) {
		pr_err_ratelimited("%s: failed to vote OFF AHB_CLIENT_ISPIF %d\n",
			__func__, rc);
		return rc;
	}
	...
	rc = cam_config_ahb_clk(NULL, 0, CAM_AHB_CLIENT_VFE0,
			CAM_AHB_SUSPEND_VOTE);
	if (rc < 0) {
		pr_err_ratelimited("%s: failed to vote OFF AHB_CLIENT_VFE0 %d\n",
			__func__, rc);
		return rc;
	}
...
int msm_ais_enable_clocks(void)
{
	...
	if (rc < 0) {
		pr_err_ratelimited("%s: clk enable failed %d\n", __func__, rc);
		rc = 0;
		return rc;
	}
...
static int msm_early_cam_probe(struct platform_device *pdev)
{
	...
	if (rc < 0) {
		pr_err_ratelimited("%s: msm_early_cam_get_clk_info() failed", __func__);
	...
	if (rc < 0)
		pr_err_ratelimited("%s:%d early_cam config_vreg failed\n", __func__,
			__LINE__);
	...
	rc = msm_camera_enable_vreg(&new_early_cam_dev->pdev->dev,
		...
		&new_early_cam_dev->early_cam_reg_ptr[0], 1);
	if (rc < 0)
		pr_err_ratelimited("%s:%d early_cam enable_vreg failed\n", __func__,
		__LINE__);

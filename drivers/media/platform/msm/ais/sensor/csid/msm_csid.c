...
static int msm_csid_config(struct csid_device *csid_dev)
{
	...
	if (!csidbase) {
		pr_err_ratelimited("%s:%d csidbase %pK\n", __func__,
			__LINE__, csidbase);
		return -EINVAL;
	}
...
static int32_t msm_csid_cmd32(struct csid_device *csid_dev, void *arg)
{
	...
	if (!csid_dev || !arg32) {
		pr_err_ratelimited("%s:%d csid_dev %pK, arg32 %pK\n", __func__, __LINE__,
			csid_dev, arg32);
		return -EINVAL;
	}
		...
		u32 evt_id, bool subscribe_flag)
{
	...
	if (rc != 0) {
		pr_err_ratelimited("%s: Subs event_type =0x%x failed\n",
				__func__, sub->type);
		return rc;
	}
	return rc;
	...
	struct v4l2_event_subscription *sub, bool subscribe_flag)
{
	...
	if (evt_mask == CSID_EVENT_SUBS_MASK_NONE) {
		pr_err_ratelimited("%s: Subs event_type is None=0x%x\n",
			__func__, evt_mask);
		return 0;
	}
	...
	if (evt_mask & (1<<evt_mask_index)) {
		...
		if (rc != 0) {
			pr_err_ratelimited("%s: Subs event index:%d failed\n",

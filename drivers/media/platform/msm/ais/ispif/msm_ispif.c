	...
	struct msm_ispif_param_data *params)
{
	...
	for (i = 0; i < params->num; i++) {
		...
		if (rc < 0)
			pr_err_ratelimited("ISPIF stop frame boundary timeout\n");
	}
	...
	struct ispif_irq_status *out, enum msm_ispif_vfe_intf vfe_id)
{
	...
	if (out[vfe_id].ispifIrqStatus0 &
		...
		if (ispif->ispif_sof_debug < ISPIF_SOF_DEBUG_COUNT)
			pr_err_ratelimited("%s: PIX0 frame id: %u irqstatus0 0x%x, irqstatus1 0x%x\n",
				__func__,
...
static long msm_ispif_cmd(struct v4l2_subdev *sd, void *arg)
{
	...
	case ISPIF_INIT:
		...
		if (rc) {
			pr_err_ratelimited("%s: %d Init failed\n", __func__, __LINE__);
			break;
		}
		msm_ispif_io_dump_reg(ispif);
		break;

...
int lge_mdss_ambient_cmd_send(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		lge_mdss_ambient_partial_area_set(pinfo, pinfo->yres);
...
int lge_mdss_ambient_create_sysfs(struct class *panel)
{
	...
	if (!sysfs_dev) {
		...
		if (IS_ERR(sysfs_dev)) {
			...
			if (device_create_file(sysfs_dev, &dev_attr_aod_interface) < 0)
...
void lge_mdss_ambient_parse_dt(struct device_node *np, struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	struct mdss_panel_info *pinfo = NULL;
	struct ambient_ctrl_info *ainfo;
	u32 tmp = 0;
	...
	rc = of_property_read_u32(np, "lge,aod-interface-data", &tmp);
	if (rc) {
		ainfo->aod_interface = 0;
		pr_err("fail to parse lge,aod-interface-data\n");
	} else {
		ainfo->aod_interface = tmp;

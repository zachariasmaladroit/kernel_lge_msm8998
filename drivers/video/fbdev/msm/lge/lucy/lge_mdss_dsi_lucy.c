...
#if 0 		/*TODO: implement  for panel shutdown*/
...
int mdss_dsi_panel_power_on(struct mdss_panel_data *pdata)
{
	...
	if (ctrl_pdata->panel_power_is_on == 1) {
		pr_err("[Display] %s is skipped. power is already on\n", __func__);
		return ret;
	} else {
		pr_err("[Display] %s ++\n", __func__);
		...
		ctrl_pdata->panel_power_is_on = 1;
...
int mdss_dsi_panel_power_off(struct mdss_panel_data *pdata)
{
	...
	/* TODO : mdss_dsi_panel_power_off  is not execute.
	   *            It must be checked  after new panel come.
	   *  	   if (ctrl_pdata->panel_power_is_on == 0 || ctrl_pdata->panel_power_is_on == 1) {
	 */
	if (ctrl_pdata->panel_power_is_on == 0 || ctrl_pdata->panel_power_is_on == 1) {
		pr_err("[Display] %s is skipped.\n", __func__);
		return ret;
	} else {
		ret = mdss_dsi_panel_reset(pdata, 0);
		if (ret) {

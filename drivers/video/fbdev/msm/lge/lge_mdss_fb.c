		...
		struct device_attribute *attr,const char *buf,  size_t count)
{
    ...
    if (enable && !lge_block_bl_update) {
        ...
        mdss_fb_set_backlight(mfd, lge_br_to_bl(mfd, 0));
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	char *envp1[2] = {"PANEL_FREEZE=1", NULL};
	char *envp0[2] = {"PANEL_FREEZE=0", NULL};
...
static DEVICE_ATTR(freeze_state_hal, S_IWUSR|S_IRUGO, freeze_state_hal_show, freeze_state_hal_store);
#endif

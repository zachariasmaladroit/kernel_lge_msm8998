	...
	if (atomic_read(&ts->state.fb) == FB_SUSPEND) {
		...
		if (ts->lpwg.screen) {
			...
			if (d->lcd_mode == LCD_MODE_U0) {
				...
				/* AOD Swipe Only */
				TOUCH_I("Swipe Only!! d->lcd_mode == %d , ts->lpwg.sensor == %d\n",
					d->lcd_mode, ts->lpwg.sensor);
...
static int sw49407_suspend(struct device *dev)
{
	...
	TOUCH_TRACE();
	d->lcd_mode = LCD_MODE_U0; //[Bringup] for knock-on
...
static int sw49407_resume(struct device *dev)
{
	...
	TOUCH_TRACE();
	d->lcd_mode = LCD_MODE_U3; //[Bringup] for knock-on
...
int sw49407_irq_handler(struct device *dev)
{
	...
	sw49407_reg_read(dev, tc_ic_status, &d->info, sizeof(d->info));
...
//[Bring-Up]		sw49407_te_test_logging(d->dev, buf);
...
static ssize_t store_burst_test(struct device *dev, const char *buf,
								size_t count)
{
	struct sw49407_reg_info temp_info;
	u32 temp_buf[11] = {0,};
	u16 index = 0;
	u16 reg_info_addr = 0x264;
	int value = 0;

	if (sscanf(buf, "%d", &value) <= 0)
		return count;

	if (!value) { // value = 0, burst mode test
		if (sw49407_reg_read(dev, reg_info_addr, &temp_info,
					sizeof(struct sw49407_reg_info)) < 0) {
			TOUCH_E("Reg info Read Error\n");
			return -1;
		}
		TOUCH_I("Burst mode Test !!!\n");
		TOUCH_I("r_info_ptr_spi_addr 0x%02X\n", temp_info.r_info_ptr_spi_addr);
		TOUCH_I("r_tc_cmd_spi_addr 0x%02X\n", temp_info.r_tc_cmd_spi_addr);
		TOUCH_I("r_watch_cmd_spi_addr 0x%02X\n", temp_info.r_watch_cmd_spi_addr);
		TOUCH_I("r_test_cmd_spi_addr 0x%02X\n", temp_info.r_test_cmd_spi_addr);
		TOUCH_I("r_abt_cmd_spi_addr 0x%02X\n", temp_info.r_abt_cmd_spi_addr);
		TOUCH_I("r_cfg_c_sram_oft 0x%02X\n", temp_info.r_cfg_c_sram_oft);
		TOUCH_I("r_cfg_s_sram_oft 0x%02X\n", temp_info.r_cfg_s_sram_oft);
		TOUCH_I("r_sys_buf_sram_oft 0x%02X\n", temp_info.r_sys_buf_sram_oft);
		TOUCH_I("r_abt_buf_sram_oft 0x%02X\n", temp_info.r_abt_buf_sram_oft);
		TOUCH_I("r_dbg_buf_sram_oft 0x%02X\n", temp_info.r_dbg_buf_sram_oft);
		TOUCH_I("r_ic_status_spi_addr 0x%02X\n", temp_info.r_ic_status_spi_addr);
		TOUCH_I("r_tc_status_spi_addr 0x%02X\n", temp_info.r_tc_status_spi_addr);
		TOUCH_I("r_abt_report_spi_addr 0x%02X\n", temp_info.r_abt_report_spi_addr);
		TOUCH_I("r_reserv_spi_addr 0x%02X\n", temp_info.r_reserv_spi_addr);
		TOUCH_I("r_chip_info_spi_addr 0x%02X\n", temp_info.r_chip_info_spi_addr);
		TOUCH_I("r_reg_info_spi_addr 0x%02X\n", temp_info.r_reg_info_spi_addr);
		TOUCH_I("r_pt_info_spi_addr 0x%02X\n", temp_info.r_pt_info_spi_addr);
		TOUCH_I("r_tc_sts_spi_addr 0x%02X\n", temp_info.r_tc_sts_spi_addr);
		TOUCH_I("r_abt_sts_spi_addr 0x%02X\n", temp_info.r_abt_sts_spi_addr);
		TOUCH_I("r_tune_code_spi_addr 0x%02X\n", temp_info.r_tune_code_spi_addr);
		TOUCH_I("r_aod_spi_addr 0x%02X\n", temp_info.r_aod_spi_addr);
	}
	else { // value = 1, byte mode test
		TOUCH_I("Byte mode Test !!!\n");
		for (index = 0; index < 11; index++) {
			sw49407_reg_read(dev, reg_info_addr + index, &temp_buf[index], sizeof(u32));
			TOUCH_I("Reg[0x%02X] : 0x%04X\n", reg_info_addr + index, temp_buf[index]);
		}
	}

/*
 * mausb_in.c - handling MAUSB IN transfers.
 *
...
void mausbdev_in_stub_complete(struct urb *urb)
{
	...
	DBG_MAUSB(DBG_LEVEL_MEDIUM,DATA_TRANS_IN, "---> %s", __func__);
	DBG_MAUSB(DBG_LEVEL_MEDIUM,DATA_TRANS_IN,
			"complete! status %d\n", urb->status);

	switch (urb->status) {
	case 0:
		/* OK */
		//DBG_MAUSB(DBG_LEVEL_MEDIUM,DATA_TRANS_IN,"OK\n");
		break;
	case -ENOENT:
		DBG_MAUSB(DBG_LEVEL_MEDIUM,DATA_TRANS_IN,
	...
	if (urb->status == -EPIPE)
		...
		if (pal->urb->status != 0)
			DBG_MAUSB(DBG_LEVEL_MEDIUM,DATA_TRANS_IN,
				"pal->urb->status: 0x%x",pal->urb->status);
	}
	DBG_MAUSB(DBG_LEVEL_MEDIUM,DATA_TRANS_IN,
				 ...
				 struct stub_mausb_pal *pal)
{
	...
	if (usb_pipeisoc(pipe)) {
		/* priv->urb = usb_alloc_urb(
			pdu->u.cmd_submit.number_of_packets, GFP_KERNEL);
		*/
	} else {
	...
	if (ep_num == USB_ENDPOINT_XFER_CONTROL) {
		pal->urb->setup_packet = kmemdup((void *)
	...
	if (!usb_pipeisoc(pipe)) {
		if (pal->urb->setup_packet && spacket) {
			pal->urb->transfer_buffer =
				kzalloc(spacket->wLength, GFP_KERNEL);
			pal->urb->transfer_buffer_length  = spacket->wLength;
		}
		else {
			pal->urb->transfer_buffer_length =
				header.u.non_iso_hdr.remsize_credit -
				sizeof(struct mausb_header);

			DBG_MAUSB(DBG_LEVEL_MEDIUM,DATA_TRANS_IN,
				"pal->urb->transfer_buffer_length: %d",pal->urb->transfer_buffer_length);

			if(pal->urb->transfer_buffer_length > 32768)

			{
				DBG_MAUSB(DBG_LEVEL_MEDIUM,DATA_TRANS_IN,
				"transfer buffer is more than 32k allocate 32k only");
				pal->urb->transfer_buffer_length=32768;
				pal->urb->transfer_buffer = kzalloc( 32768,GFP_KERNEL);
			}
			else
			{

			pal->urb->transfer_buffer =
				kzalloc( header.u.non_iso_hdr.remsize_credit -
					sizeof(struct mausb_header),
					GFP_KERNEL);
			}
	...
	if (ret == 0) {
		...
		DBG_MAUSB(DBG_LEVEL_MEDIUM,DATA_TRANS_IN,
			"IN: Time taken for urb submission:"
	...
	DBG_MAUSB(DBG_LEVEL_MEDIUM,DATA_TRANS_IN, "<-- %s\n",__func__);
	return;
}

static struct stub_mausb_pal *dequeue_from_mausb_pal_in_init(

...
{
	return (MAUSB_FLAG_HOST << MAUSB_FLAG_OFFSET) &
		header->base.flags_version;
}

int mausb_is_mgmt_pkt(struct mausb_header *header)
{
	return MAUSB_PKT_TYPE_MGMT == (header->base.type_subtype &
			MAUSB_PKT_TYPE_MASK);
}

int mausb_is_data_pkt(struct mausb_header *header)
{
	return MAUSB_PKT_TYPE_DATA == (header->base.type_subtype &
			MAUSB_PKT_TYPE_MASK);
}
...
unsigned char mausb_get_uint8(unsigned char *buff, const int offset,
		const int length)
{
	if (offset < 0 || !buff)
		return 0;
...
unsigned short mausb_get_uint16(unsigned char *buff, const int offset,
		const int length)
{
	unsigned short *ptr;
...
unsigned int mausb_get_uint24(unsigned char *buff, const int offset,
		const int length)
{
	unsigned int *ptr;
	unsigned int val;

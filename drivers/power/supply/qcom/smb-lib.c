...
static void smblib_step_charging_check_work(struct work_struct *work)
{
	...
	if (is_step_up && chg->sc_level < sc_size_temp) {
		...
		queue_delayed_work(system_power_efficient_wq, &chg->step_charging_check_work,
...
irqreturn_t smblib_handle_aicl_fail(int irq, void *data)
{
	...
	queue_delayed_work(system_power_efficient_wq, &chg->aicl_fail_wa_work,
...
static void weak_batt_pack_check_work(struct work_struct *work)
{
	...
	if (current_time == last_time) {
		...
		queue_delayed_work(system_power_efficient_wq, &chg->batt_pack_check_work,
...
#ifdef CONFIG_LGE_PM
		cancel_delayed_work_sync(&chg->recovery_boost_back_work);
		queue_delayed_work(system_power_efficient_wq, &chg->recovery_boost_back_work,
...
static void vzw_recheck_work(struct work_struct *work)
{
	...
	queue_delayed_work(system_power_efficient_wq, &chg->vzw_aicl_done_work, 0);
}
...
static void abnormal_gender_detect_work(struct work_struct *work){
	...
	queue_delayed_work(system_power_efficient_wq, &chg->slow_charging_detect_work,
...
static void charging_information(struct work_struct *work)
{
	...
	queue_delayed_work(system_power_efficient_wq, &chg->charging_inform_work,

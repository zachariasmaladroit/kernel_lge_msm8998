...
#define LGTTF_ID_POWERON_SIGNATURE    2
...
static int lgttf_upperbound(void)
{
	...
	else if (lge_get_hydra_mode() == LGE_HYDRA_MODE_SIGNATURE)
		return lgttf.table[LGTTF_ID_POWERON_SIGNATURE].power;
	else
...
static int lgttf_find_table(int power)
{
	...
	if (lgttf.rt.is_wireless)
		type = LGTTF_TYPE_WIRELESS;

	if (lgttf.rt.is_chargerlogo)
		begin = LGTTF_ID_CHARGERLOGO;
	else if (lge_get_hydra_mode() == LGE_HYDRA_MODE_SIGNATURE)
		begin = LGTTF_ID_POWERON_SIGNATURE;
...
static void lgttf_work_func(struct work_struct *work)
{
	...
	if (lgttf.rt.power != power) {
		pr_chgtime(UPDATE,
			"One more sampling (power: %dmW)\n", power);

...
static int __init setup_bootmode_plmn(const char* s) {
	if (!strcmp(s, "KT"))
...
bool unified_bootmode_create(struct device_node* dnode) {
	const char* plmn_name;
	pr_bootmod("Test get boot mode %s, (%d)\n",
		dnode ? "Y":"N", bootmode_region);

	if (dnode && bootmode_region == REGION_COM) {
		of_property_read_string(dnode, "lge,plmn-name", &plmn_name);
		setup_bootmode_plmn(plmn_name);
	}
	return true;
};

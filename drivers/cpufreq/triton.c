...
static int get_opt_frequency(int cl, int lvl)
{
	...
	pol = cpufreq_interactive_get_policy(fcpu);

	if(!pol) {
		pr_err("policy is NULL skip this\n");
		return freq;
	}

	spin_lock_irqsave(&load_lock, flags[0]);

	cpufreq_lock(fcpu, flags[1]);
	sched_get_cpus_busy(&schedl[0], pol->cpus);
	cpufreq_unlock(fcpu, flags[1]);
	...
	trace_cpufreq_triton_level(platform_data.notify_info.cur_policy,
